...
function BasketList(props) {
    const { order = [], handleBasketShow = Function.prototype, deleteFromBasket = Function.prototype } = props
    ...
    return  <ul className="collection with-header basket-list">
        <li className="collection-header"><h4>Корзина</h4></li>
        {
            order.length ? order.map(item => (
                <BasketItem key={item.id} deleteFromBasket={deleteFromBasket} {...item} />
